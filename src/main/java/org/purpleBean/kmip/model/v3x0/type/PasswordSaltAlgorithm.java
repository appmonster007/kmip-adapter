...
public class PasswordSaltAlgorithm implements KmipDataType {
  ...
  static {
    for (KmipSpec spec : supportedVersions) {
      ...
      KmipEnumeration.register(spec, kmipTag.getValue(), PasswordSaltAlgorithm::fromName,
          PasswordSaltAlgorithm::fromValue);
    }
  }
  ...
  private void validate() {
    ...
    if (!value.isSupported()) {
      throw new IllegalArgumentException(
          String.format("Value '%s' for PasswordSaltAlgorithm is not supported for KMIP spec %s",
