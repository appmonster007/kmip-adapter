 ...
 *
 * <p>Spec: {@code Rotate Name Value} (Text String, required) + {@code Rotate Name Type} (Enumeration, required).
 *
...
public class RotateName implements KmipStructure, KmipAttribute {
    ...
    static {
        for (KmipSpec spec : supportedVersions) {
            ...
            KmipAttribute.register(spec, kmipTag.getValue(), encodingType, RotateName.class, RotateName::of);
        }
    }
    ...
    @Builder
    private RotateName(@NonNull RotateNameValue rotateNameValue, @NonNull RotateNameType rotateNameType) {
        this.rotateNameValue = rotateNameValue;
        this.rotateNameType = rotateNameType;
        validate();
    }

    public static RotateName of(@NonNull RotateNameValue rotateNameValue, @NonNull RotateNameType rotateNameType) {
        return new RotateName(rotateNameValue, rotateNameType);
    }

    public static RotateName of(@NonNull AttributeName attributeName, @NonNull AttributeValue attributeValue) {
        ...
        return RotateName.builder()
                .rotateNameValue((RotateNameValue) map.get(RotateNameValue.kmipTag).get(0))
                .rotateNameType((RotateNameType) map.get(RotateNameType.kmipTag).get(0))
                .build();
    }
    ...
    private void validate() {
        ...
        if (!rotateNameValue.isSupported()) {
            throw new IllegalArgumentException(String.format("RotateNameValue is not supported for KMIP spec %s", spec));
        }
        if (!rotateNameType.isSupported()) {
            throw new IllegalArgumentException(String.format("RotateNameType is not supported for KMIP spec %s", spec));
        }
    }
    ...
    public KmipDataType[] getValue() {
        return Stream.of(rotateNameValue, rotateNameType)
