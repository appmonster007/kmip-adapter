...
public class ProfileInformation implements KmipStructure {
    ...
    public static ProfileInformation of(@NonNull ProfileName profileName) {
        return ProfileInformation.builder().profileName(profileName).build();
    }
    ...
    public static ProfileInformation of(@NonNull KmipDataType value) {
        ...
        for (KmipDataType field : structure.getValue()) {
            if (field instanceof ProfileName n) builder.profileName(n);
            else if (field instanceof ProfileVersion v) builder.profileVersion(v);
            else if (field instanceof ServerUri u) builder.serverUri(u);
            else if (field instanceof ServerPort p) builder.serverPort(p);
        }
    ...
    public KmipDataType[] getValue() {
        return Stream.of(profileName, profileVersion, serverUri, serverPort)
