...
public class ResponseBatchItem implements ResponseBatchItemStructure {
    ...
    public static ResponseBatchItem of(List<KmipDataType> values) {
        ...
        if (map.containsKey(Operation.kmipTag)) builder.operation((Operation) map.get(Operation.kmipTag).getFirst());
        if (map.containsKey(UniqueBatchItemID.kmipTag)) builder.uniqueBatchItemID((UniqueBatchItemID) map.get(UniqueBatchItemID.kmipTag).getFirst());
        if (map.containsKey(ResultStatus.kmipTag)) builder.resultStatus((ResultStatus) map.get(ResultStatus.kmipTag).getFirst());
        if (map.containsKey(ResultReason.kmipTag)) builder.resultReason((ResultReason) map.get(ResultReason.kmipTag).getFirst());
        if (map.containsKey(ResultMessage.kmipTag)) builder.resultMessage((ResultMessage) map.get(ResultMessage.kmipTag).getFirst());
        if (map.containsKey(AsynchronousCorrelationValue.kmipTag)) builder.asynchronousCorrelationValue((AsynchronousCorrelationValue) map.get(AsynchronousCorrelationValue.kmipTag).getFirst());
        if (map.containsKey(ResponsePayloadStructure.kmipTag)) builder.responsePayloadStructure((ResponsePayloadStructure) map.get(ResponsePayloadStructure.kmipTag).getFirst());
        if (map.containsKey(MessageExtension.kmipTag)) builder.messageExtension((MessageExtension) map.get(MessageExtension.kmipTag).getFirst());
