...
public class CapabilityInformation implements KmipStructure {
    ...
    @Builder
    private CapabilityInformation(StreamingCapability streamingCapability, AsynchronousCapability asynchronousCapability,
                                  AttestationCapability attestationCapability, BatchUndoCapability batchUndoCapability,
                                  BatchContinueCapability batchContinueCapability, UnwrapMode unwrapMode,
    ...
    public static CapabilityInformation of(@lombok.NonNull KmipDataType value) {
        ...
        for (KmipDataType field : structure.getValue()) {
            if (field instanceof StreamingCapability c) builder.streamingCapability(c);
            else if (field instanceof AsynchronousCapability c) builder.asynchronousCapability(c);
            else if (field instanceof AttestationCapability c) builder.attestationCapability(c);
            else if (field instanceof BatchUndoCapability c) builder.batchUndoCapability(c);
            else if (field instanceof BatchContinueCapability c) builder.batchContinueCapability(c);
            else if (field instanceof UnwrapMode m) builder.unwrapMode(m);
            else if (field instanceof DestroyAction a) builder.destroyAction(a);
            else if (field instanceof ShreddingAlgorithm a) builder.shreddingAlgorithm(a);
            else if (field instanceof RngMode m) builder.rngMode(m);
            else if (field instanceof QuantumSafeCapability c) builder.quantumSafeCapability(c);
        }
    ...
    public KmipDataType[] getValue() {
        return Stream.of(streamingCapability, asynchronousCapability, attestationCapability,
