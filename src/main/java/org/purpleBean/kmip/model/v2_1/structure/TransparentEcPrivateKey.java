...
public class TransparentEcPrivateKey implements KeyMaterial, KmipStructure {
    ...
    private static final KeyFormatType.Value keyFormatTypeValue = KeyFormatType.Standard.TRANSPARENT_EC_PRIVATE_KEY;

    static {
        for (KmipSpec spec : supportedVersions) {
            if (spec == KmipSpec.UnknownVersion || spec == KmipSpec.UnsupportedVersion) continue;
            KmipDataType.register(spec, kmipTag.getValue(), encodingType, TransparentEcPrivateKey.class);
            KeyMaterial.register(spec, encodingType, keyFormatTypeValue, TransparentEcPrivateKey.class, TransparentEcPrivateKey::of);
        }
    }
    ...
    public static TransparentEcPrivateKey of(@NonNull RecommendedCurve recommendedCurve, @NonNull D d) {
