...
public class TransparentEcPublicKey implements KeyMaterial, KmipStructure {
    ...
    private static final KeyFormatType.Value keyFormatTypeValue = KeyFormatType.Standard.TRANSPARENT_EC_PUBLIC_KEY;

    static {
        for (KmipSpec spec : supportedVersions) {
            if (spec == KmipSpec.UnknownVersion || spec == KmipSpec.UnsupportedVersion) continue;
            KmipDataType.register(spec, kmipTag.getValue(), encodingType, TransparentEcPublicKey.class);
            KeyMaterial.register(spec, encodingType, keyFormatTypeValue, TransparentEcPublicKey.class, TransparentEcPublicKey::of);
        }
    }
    ...
    public static TransparentEcPublicKey of(@NonNull RecommendedCurve recommendedCurve, @NonNull QString qString) {
