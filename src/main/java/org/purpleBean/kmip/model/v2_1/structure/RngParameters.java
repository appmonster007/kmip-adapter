...
public class RngParameters implements KmipStructure {
    ...
    public static RngParameters of(@NonNull RngAlgorithm rngAlgorithm) {
        return RngParameters.builder().rngAlgorithm(rngAlgorithm).build();
    }
    ...
    public static RngParameters of(@NonNull KmipDataType value) {
        ...
        for (KmipDataType field : structure.getValue()) {
            if (field instanceof RngAlgorithm a) builder.rngAlgorithm(a);
            else if (field instanceof CryptographicAlgorithm a) builder.cryptographicAlgorithm(a);
            else if (field instanceof CryptographicLength l) builder.cryptographicLength(l);
            else if (field instanceof HashingAlgorithm a) builder.hashingAlgorithm(a);
            else if (field instanceof DrbgAlgorithm a) builder.drbgAlgorithm(a);
            else if (field instanceof RecommendedCurve c) builder.recommendedCurve(c);
            else if (field instanceof Fips186Variation v) builder.fips186Variation(v);
            else if (field instanceof PredictionResistance p) builder.predictionResistance(p);
        }
    ...
    public KmipDataType[] getValue() {
        return Stream.of(rngAlgorithm, cryptographicAlgorithm, cryptographicLength,
                        hashingAlgorithm, drbgAlgorithm, recommendedCurve, fips186Variation, predictionResistance)
