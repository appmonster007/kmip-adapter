...
public class ValidationInformation implements KmipStructure {
    ...
    private ValidationInformation(@NonNull ValidationAuthorityType validationAuthorityType,
        ...
        this.validationProfiles = (validationProfiles == null) ? Collections.emptyList() : validationProfiles;
        validate();
    }
    ...
    public static ValidationInformation of(@NonNull KmipDataType value) {
        ...
        for (KmipDataType field : structure.getValue()) {
            if (field instanceof ValidationAuthorityType t) builder.validationAuthorityType(t);
            else if (field instanceof ValidationAuthorityCountry c) builder.validationAuthorityCountry(c);
            else if (field instanceof ValidationAuthorityUri u) builder.validationAuthorityUri(u);
            else if (field instanceof ValidationVersionMajor m) builder.validationVersionMajor(m);
            else if (field instanceof ValidationVersionMinor m) builder.validationVersionMinor(m);
            else if (field instanceof ValidationType t) builder.validationType(t);
            else if (field instanceof ValidationLevel l) builder.validationLevel(l);
            else if (field instanceof ValidationCertificateIdentifier i) builder.validationCertificateIdentifier(i);
            else if (field instanceof ValidationCertificateUri u) builder.validationCertificateUri(u);
            else if (field instanceof ValidationVendorUri u) builder.validationVendorUri(u);
            else if (field instanceof ValidationProfile p) builder.validationProfile(p);
        }
