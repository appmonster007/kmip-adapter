...
/**
 * KMIP GetConstraints Response Payload (stub). Field-level design pending — currently registers as an
 * empty structure so the codec registry is populated; downstream work needs to add the
 * actual request fields per OASIS KMIP spec for GetConstraints.
 */
@Data
...
public class GetConstraintsOpResponsePayload implements ResponsePayloadStructure {
    ...
    static {
        for (KmipSpec spec : supportedVersions) {
            ...
            KmipDataType.register(spec, kmipTag.getValue(), encodingType, GetConstraintsOpResponsePayload.class);
            ResponsePayloadStructure.register(spec, operation, GetConstraintsOpResponsePayload.class, GetConstraintsOpResponsePayload::of);
        }
    }

    @Builder
    private GetConstraintsOpResponsePayload() {
        validate();
    }
