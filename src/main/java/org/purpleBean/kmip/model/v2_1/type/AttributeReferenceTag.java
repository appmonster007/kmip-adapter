...
/**
 * Represents an AttributeReference encoded as an ENUMERATION — references a standard attribute by its tag.
...
public class AttributeReferenceTag implements KmipDataType {
    ...
    /**
     * Returns a KmipEnumeration.Value whose description is the attribute tag description ("Name", etc.).
    ...
    public Object getValue() {
        return new KmipEnumeration.Value<AttributeReferenceTag>() {
            @Override
            public int getValue() {
                return KmipTag.fromName(tagDescription).getValue();
            }
