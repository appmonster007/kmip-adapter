...
public class Objects implements KmipStructure {
    ...
    public static Objects of(@NonNull List<UniqueIdentifier> uniqueIdentifiers) {
        return Objects.builder().uniqueIdentifiers(uniqueIdentifiers).build();
    }
    ...
    public boolean isSupported() {
        ...
        return supportedVersions.contains(spec) && uniqueIdentifiers.stream().allMatch(KmipDataType::isSupported);
    }

    @Override
    public KmipDataType[] getValue() {
        return uniqueIdentifiers.stream().map(KmipDataType.class::cast).toArray(KmipDataType[]::new);
    }
}
