...
public class ObjectGroups implements KmipStructure {
    ...
    public static ObjectGroups of(@NonNull List<ObjectGroup> objectGroups) {
        return ObjectGroups.builder().objectGroups(objectGroups).build();
    }
    ...
    public boolean isSupported() {
        ...
        return supportedVersions.contains(spec) && objectGroups.stream().allMatch(KmipDataType::isSupported);
    }

    @Override
    public KmipDataType[] getValue() {
        return objectGroups.stream().map(KmipDataType.class::cast).toArray(KmipDataType[]::new);
    }
}
