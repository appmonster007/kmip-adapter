...
/**
 * KMIP Logout Response Payload (stub). Field-level design pending — currently registers as an
 * empty structure so the codec registry is populated; downstream work needs to add the
 * actual request fields per OASIS KMIP spec for Logout.
 */
@Data
@Builder(toBuilder = true)
public class LogoutOpResponsePayload implements ResponsePayloadStructure {
    ...
    static {
        for (KmipSpec spec : supportedVersions) {
            ...
            KmipDataType.register(spec, kmipTag.getValue(), encodingType, LogoutOpResponsePayload.class);
            ResponsePayloadStructure.register(spec, operation, LogoutOpResponsePayload.class, LogoutOpResponsePayload::of);
        }
    }

    @Builder
    private LogoutOpResponsePayload() {
        validate();
    }
