...
public class ProtectionLevel implements KmipEnumeration {
    ...
    static {
        ...
        for (KmipSpec spec : supportedVersions) {
            ...
            KmipEnumeration.register(spec, kmipTag.getValue(), ProtectionLevel::fromName, ProtectionLevel::fromValue);
        }
    }
    ...
    private void validate() {
        ...
        if (!value.isSupported()) {
            throw new IllegalArgumentException(
                    String.format("Value '%s' for ProtectionLevel is not supported for KMIP spec %s", value.getDescription(), spec)
            );
        }
        if (!isSupported()) {
