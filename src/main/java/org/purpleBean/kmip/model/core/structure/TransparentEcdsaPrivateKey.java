...
public class TransparentEcdsaPrivateKey implements KeyMaterial, KmipStructure {
    ...
    private static final KeyFormatType.Value keyFormatTypeValue = KeyFormatType.Standard.TRANSPARENT_ECDSA_PRIVATE_KEY;

    static {
        for (KmipSpec spec : supportedVersions) {
            if (spec == KmipSpec.UnknownVersion || spec == KmipSpec.UnsupportedVersion) continue;
            KeyMaterial.register(spec, encodingType, keyFormatTypeValue, TransparentEcdsaPrivateKey.class, TransparentEcdsaPrivateKey::of);
        }
    }
    ...
    public KmipDataType[] getValue() {
        return Stream.of(recommendedCurve, d)
