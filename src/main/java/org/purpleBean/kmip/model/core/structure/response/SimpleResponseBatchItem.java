...
public class SimpleResponseBatchItem implements ResponseBatchItemStructure {
    ...
    static {
        KmipDataType.register(KmipSpec.UnknownVersion, kmipTag.getValue(), encodingType, SimpleResponseBatchItem.class);
        ResponseBatchItemStructure.register(KmipSpec.UnknownVersion, SimpleResponseBatchItem.class, SimpleResponseBatchItem::of);
    }
    ...
    public static SimpleResponseBatchItem of(List<KmipDataType> values) {
        ...
        if (map.containsKey(ResultStatus.kmipTag)) {
            builder.resultStatus((ResultStatus) map.get(ResultStatus.kmipTag).get(0));
        }
        if (map.containsKey(ResultReason.kmipTag)) {
            builder.resultReason((ResultReason) map.get(ResultReason.kmipTag).get(0));
        }
        if (map.containsKey(ResultMessage.kmipTag)) {
            builder.resultMessage((ResultMessage) map.get(ResultMessage.kmipTag).get(0));
        }
        if (map.containsKey(ResponsePayloadStructure.kmipTag)) {
            builder.responsePayloadStructure((ResponsePayloadStructure) map.get(ResponsePayloadStructure.kmipTag).get(0));
        }
