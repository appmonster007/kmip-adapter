...
public class SplitKeyPolynomial implements KmipEnumeration {
    public static final KmipTag kmipTag = KmipTag.Standard.SPLIT_KEY_POLYNOMIAL.inst();
    ...
    static {
        ...
        for (KmipSpec spec : supportedVersions) {
            ...
            KmipDataType.register(spec, kmipTag.getValue(), encodingType, SplitKeyPolynomial.class);
            KmipEnumeration.register(spec, kmipTag.getValue(), SplitKeyPolynomial::fromName, SplitKeyPolynomial::fromValue);
        }
    }
    ...
    @Builder
    private SplitKeyPolynomial(@NonNull Value value) {
        this.value = value;
        validate();
    }
    ...
    public static Value fromName(String name) {
        ...
        return Optional.ofNullable(v)
                ...
                .orElseThrow(() -> new NoSuchElementException(
                        String.format("No SplitKeyPolynomial value found for '%s' in KMIP spec %s", name, spec)
                ));
    }
    ...
    public static Value fromValue(int value) {
        ...
        return Optional.ofNullable(v)
                .filter(Value::isSupported)
                .orElseThrow(() -> new NoSuchElementException(
                        String.format("No SplitKeyPolynomial value found for %d in KMIP spec %s", value, spec)
                ));
    }
    ...
    private void validate() {
        ...
        if (!value.isSupported()) {
            throw new IllegalArgumentException(
                    String.format("Value '%s' for SplitKeyPolynomial is not supported for KMIP spec %s", value.getDescription(), spec)
            );
        }
        if (!isSupported()) {
    ...
    public enum Standard implements Value {
        POLYNOMIAL_283(0x00000001, "Polynomial283", KmipSpec.UnknownVersion, KmipSpec.V3_0),
        POLYNOMIAL_285(0x00000002, "Polynomial285", KmipSpec.UnknownVersion, KmipSpec.V3_0);
     ...
     */
    public interface Value extends KmipEnumeration.Value<SplitKeyPolynomial> {
    }
