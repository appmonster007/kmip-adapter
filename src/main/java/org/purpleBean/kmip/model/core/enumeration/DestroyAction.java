 ...
 *   <li>{@code KEY_MATERIAL_DELETED}: The key material is deleted, but the metadata may be
 *   retained.</li>
 *   <li>{@code KEY_MATERIAL_SHREDDED}: The key material is securely shredded.</li>
 *   <li>{@code META_DATA_DELETED}: The metadata is deleted, but the key material may be retained
 *   .</li>
...
public class DestroyAction implements KmipEnumeration {
  ...
  static {
    ...
    for (KmipSpec spec : supportedVersions) {
      ...
      KmipEnumeration.register(spec, kmipTag.getValue(), DestroyAction::fromName,
          DestroyAction::fromValue);
    }
  }
  ...
  private void validate() {
    ...
    if (!value.isSupported()) {
      throw new IllegalArgumentException(
          String.format("Value '%s' for DestroyAction is not supported for KMIP spec %s",
   ...
   */
  public interface Value extends KmipEnumeration.Value<DestroyAction> {
  }
