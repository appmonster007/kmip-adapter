...
public class ClientRegistrationMethod implements KmipEnumeration {
    ...
    static {
        ...
        for (KmipSpec spec : supportedVersions) {
            ...
            KmipEnumeration.register(spec, kmipTag.getValue(), ClientRegistrationMethod::fromName, ClientRegistrationMethod::fromValue);
        }
    }
    ...
    public static Value fromName(String name) {
        ...
        return Optional.ofNullable(v)
                ...
                .orElseThrow(() -> new NoSuchElementException(
                        String.format("No ClientRegistrationMethod value found for '%s' in KMIP spec %s", name, spec)
                ));
    }
    ...
    public static Value fromValue(int value) {
        ...
        return Optional.ofNullable(v)
                .filter(Value::isSupported)
                .orElseThrow(() -> new NoSuchElementException(
                        String.format("No ClientRegistrationMethod value found for %d in KMIP spec %s", value, spec)
                ));
    }
    ...
    private void validate() {
        ...
        if (!value.isSupported()) {
            throw new IllegalArgumentException(
                    String.format("Value '%s' for ClientRegistrationMethod is not supported for KMIP spec %s", value.getDescription(), spec)
            );
        }
        if (!isSupported()) {
