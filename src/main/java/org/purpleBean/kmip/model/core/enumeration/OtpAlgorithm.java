...
public class OtpAlgorithm implements KmipEnumeration {
    public static final KmipTag kmipTag = KmipTag.Standard.OTP_ALGORITHM.inst();
    ...
    static {
        ...
        for (KmipSpec spec : supportedVersions) {
            ...
            KmipDataType.register(spec, kmipTag.getValue(), encodingType, OtpAlgorithm.class);
            KmipEnumeration.register(spec, kmipTag.getValue(), OtpAlgorithm::fromName, OtpAlgorithm::fromValue);
        }
    }
    ...
    @Builder
    private OtpAlgorithm(@NonNull Value value) {
        this.value = value;
        validate();
    }
    ...
    public static Value fromName(String name) {
        ...
        return Optional.ofNullable(v)
                ...
                .orElseThrow(() -> new NoSuchElementException(
                        String.format("No OtpAlgorithm value found for '%s' in KMIP spec %s", name, spec)
                ));
    }
    ...
    public static Value fromValue(int value) {
        ...
        return Optional.ofNullable(v)
                .filter(Value::isSupported)
                .orElseThrow(() -> new NoSuchElementException(
                        String.format("No OtpAlgorithm value found for %d in KMIP spec %s", value, spec)
                ));
    }
    ...
    private void validate() {
        ...
        if (!value.isSupported()) {
            throw new IllegalArgumentException(
                    String.format("Value '%s' for OtpAlgorithm is not supported for KMIP spec %s", value.getDescription(), spec)
            );
        }
        if (!isSupported()) {
    ...
    public enum Standard implements Value {
        HOTP(0x00000001, "Hotp", KmipSpec.UnknownVersion, KmipSpec.V3_0),
        TOTP(0x00000002, "Totp", KmipSpec.UnknownVersion, KmipSpec.V3_0);
        ...
        @Override
        public OtpAlgorithm inst() {
            return OtpAlgorithm.of(this);
        }
    }
     ...
     */
    public interface Value extends KmipEnumeration.Value<OtpAlgorithm> {
    }
