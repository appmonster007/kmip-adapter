...
public class DeviceCredential implements CredentialValue, KmipStructure {
  ...
  static {
    for (KmipSpec spec : supportedVersions) {
      ...
      CredentialValue.register(spec, encodingType, CredentialType.Standard.DEVICE,
          DeviceCredential.class, DeviceCredential::of);
    }
  }
  ...
  public static DeviceCredential of(List<KmipDataType> values) {
    ...
    if (map.containsKey(DeviceSerialNumber.kmipTag)) {
      builder.deviceSerialNumber((DeviceSerialNumber) map
          .get(DeviceSerialNumber.kmipTag)
          .getFirst());
    }
    ...
    if (map.containsKey(DeviceIdentifier.kmipTag)) {
      builder.deviceIdentifier((DeviceIdentifier) map
          .get(DeviceIdentifier.kmipTag)
          .getFirst());
    }
    if (map.containsKey(NetworkIdentifier.kmipTag)) {
      builder.networkIdentifier((NetworkIdentifier) map
          .get(NetworkIdentifier.kmipTag)
          .getFirst());
    }
    if (map.containsKey(MachineIdentifier.kmipTag)) {
      builder.machineIdentifier((MachineIdentifier) map
          .get(MachineIdentifier.kmipTag)
          .getFirst());
    }
    if (map.containsKey(MediaIdentifier.kmipTag)) {
      builder.mediaIdentifier((MediaIdentifier) map
          .get(MediaIdentifier.kmipTag)
          .getFirst());
    }
