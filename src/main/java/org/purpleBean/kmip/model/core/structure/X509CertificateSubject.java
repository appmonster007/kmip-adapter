...
public class X509CertificateSubject implements KmipStructure, KmipAttribute {
  ...
  static {
    for (KmipSpec spec : supportedVersions) {
      ...
      KmipAttribute.register(spec, kmipTag.getValue(), encodingType, X509CertificateSubject.class,
          X509CertificateSubject::of);
    }
  }
  ...
  private X509CertificateSubject(
    ...
    this.subjectAlternativeNames =
        (subjectAlternativeNames == null) ? Collections.emptyList() : subjectAlternativeNames;
    validate();
  }
  ...
  public static X509CertificateSubject of(@NonNull AttributeName attributeName,
    ...
    return X509CertificateSubject
        .builder()
        .subjectDistinguishedName((SubjectDistinguishedName) map
            .get(SubjectDistinguishedName.kmipTag)
            .get(0))
        .subjectAlternativeNames(map
            .get(SubjectAlternativeName.kmipTag)
            .stream()
            .map(e -> (SubjectAlternativeName) e)
  ...
  public KmipDataType[] getValue() {
    return Stream
        .of(subjectDistinguishedName, subjectAlternativeNames)
