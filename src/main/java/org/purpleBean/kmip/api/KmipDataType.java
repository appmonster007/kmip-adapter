...
/**
 * Represents a fundamental data type within the Key Management Interoperability Protocol (KMIP) framework.
 * <p>
 * This interface serves as the base for all specific KMIP data types, establishing a common contract for identifying
 * and handling different kinds of data in KMIP messages. It provides a centralized registry system to map protocol-specific
 * identifiers (like tags and encoding types) to their corresponding Java class representations.
 *
 * <p><b>Key Responsibilities:</b></p>
 * <ul>
 *   <li><b>Type Identification:</b> Every KMIP data type must be able to provide its {@link KmipTag} and
 *       {@link EncodingType}, which together uniquely identify it within a specific KMIP specification version.</li>
 *   <li><b>Dynamic Registration:</b> A static registry ({@code TAG_REGISTRY}) allows for the dynamic registration of
 *       KMIP data types. This mechanism maps a combination of {@link KmipSpec}, {@link KmipTag.Value}, and
 *       {@link EncodingType} to the Java class that implements the data type. This is crucial for the codec to
 *       know which class to instantiate during deserialization.</li>
 *   <li><b>Compatibility Checking:</b> The {@link #isSupported()} method allows for runtime checks to determine if a
 *       given data type is supported under the currently active {@link KmipContext}.</li>
 * </ul>
 *
 * <p><b>RegistryKey:</b></p>
 * The internal {@code RegistryKey} record is used as the key for the {@code TAG_REGISTRY}. It ensures that the mapping
 * is precise, taking into account the KMIP specification version, the tag of the data type, and its encoding.
 *
 * @see KmipStructure
 * @see KmipEnumeration
 * @see KmipAttribute
...
public interface KmipDataType {
    /**
     * A thread-safe registry that maps a unique combination of KMIP specification, tag, and encoding type
     * to the corresponding Java class implementation of a {@link KmipDataType}. This is the core mechanism
    ...
    /**
     * Retrieves the corresponding {@link KmipDataType} class from the registry based on a tag and encoding type.
     * <p>
     * This method uses the {@link KmipContext} to determine the currently active KMIP specification and looks up
     ...
     * @return The registered {@link Class} for the given parameters, or {@code null} if no mapping is found.
     */
     ...
     *
     * @return {@code true} if the data type is supported under the active {@link KmipContext}, {@code false} otherwise.
     */
    boolean isSupported();
