...
public class TransparentEcmqvPublicKey implements KeyMaterial.Structure {
    private static final Set<KmipSpec> supportedVersions = Set.of(KmipSpec.UnknownVersion, KmipSpec.V1_2);

    static {
        for (KmipSpec spec : supportedVersions) {
            if (spec == KmipSpec.UnknownVersion || spec == KmipSpec.UnsupportedVersion) continue;
            KeyMaterial.Value.register(spec, encodingType, KeyFormatType.Standard.TRANSPARENT_ECMQV_PUBLIC_KEY, TransparentEcmqvPublicKey.class, TransparentEcmqvPublicKey::of);
        }
    }
    ...
    public static TransparentEcmqvPublicKey of(@NonNull KeyMaterial.Value value) {
