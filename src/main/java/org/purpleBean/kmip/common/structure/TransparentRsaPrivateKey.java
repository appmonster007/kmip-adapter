...
public class TransparentRsaPrivateKey implements KeyMaterial.Structure {
    private static final Set<KmipSpec> supportedVersions = Set.of(KmipSpec.UnknownVersion, KmipSpec.V1_2);

    static {
        for (KmipSpec spec : supportedVersions) {
            if (spec == KmipSpec.UnknownVersion || spec == KmipSpec.UnsupportedVersion) continue;
            KeyMaterial.Value.register(spec, encodingType, KeyFormatType.Standard.TRANSPARENT_RSA_PRIVATE_KEY, TransparentRsaPrivateKey.class, TransparentRsaPrivateKey::of);
        }
    }
    ...
    public static TransparentRsaPrivateKey of(@NonNull KeyMaterial.Value value) {
