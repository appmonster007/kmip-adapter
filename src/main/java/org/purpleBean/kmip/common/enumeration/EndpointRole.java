...
public class EndpointRole implements KmipEnumeration {
    public static final KmipTag kmipTag = new KmipTag(KmipTag.Standard.ENDPOINT_ROLE);
    ...
    public EndpointRole(@NonNull Value value) {
        // KMIP spec compatibility validation
        KmipSpec spec = KmipContext.getSpec();
        if (!value.isSupported()) {
            throw new IllegalArgumentException(
                    String.format("Value '%s' for EndpointRole is not supported for KMIP spec %s", value.getDescription(), spec)
            );
        }
        this.value = value;
    }
