...
public class TicketTypeJsonDeserializer extends KmipDataTypeJsonDeserializer<TicketType> {
    ...
    public TicketType deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (node == null) {
            ctxt.reportInputMismatch(TicketType.class, String.format("JSON node cannot be null for TicketType deserialization"));
            return null;
        }
        ...
        try {
            ...
            if (tag == null) {
                ctxt.reportInputMismatch(TicketType.class, String.format("Invalid KMIP tag for TicketType"));
                return null;
            }
        } catch (Exception e) {
        ...
        if (!node.isObject() || tag.getValue().getValue() != kmipTag.getValue().getValue()) {
            ctxt.reportInputMismatch(TicketType.class,
                    String.format("Expected object with %s tag for TicketType, got tag: %s", kmipTag.getValue().getValue(), tag.getValue().getValue()));
            return null;
        }
        ...
        if (typeNode == null
            ...
            ctxt.reportInputMismatch(TicketType.class, String.format("Missing or non-text 'type' field for TicketType"));
            return null;
        }
