...
public class TransparentEcmqvPublicKeyJsonDeserializer extends KmipDataTypeJsonDeserializer<TransparentEcmqvPublicKey> {
    ...
    public TransparentEcmqvPublicKey deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (node == null) {
            ctxt.reportInputMismatch(TransparentEcmqvPublicKey.class, String.format("JSON node cannot be null for TransparentEcmqvPublicKey deserialization"));
            return null;
        }
        ...
        try {
            ...
            if (tag == null) {
                ctxt.reportInputMismatch(TransparentEcmqvPublicKey.class, String.format("Invalid KMIP tag for TransparentEcmqvPublicKey"));
                return null;
            }
        } catch (Exception e) {
            ctxt.reportInputMismatch(TransparentEcmqvPublicKey.class, String.format("Failed to parse KMIP tag for TransparentEcmqvPublicKey: %s", e.getMessage()));
            return null;
        }

        if (!node.isObject() || !tag.getValue().equals(kmipTag.getValue())) {
            ctxt.reportInputMismatch(TransparentEcmqvPublicKey.class,
                    String.format("Expected object with %s tag for TransparentEcmqvPublicKey, got tag: %s", kmipTag.getValue().getValue(), tag.getValue().getValue()));
            return null;
        }
        ...
        if (typeNode == null
            ...
            ctxt.reportInputMismatch(TransparentEcmqvPublicKey.class, String.format("Missing or non-text 'type' field for TransparentEcmqvPublicKey"));
            return null;
        }
        ...
        if (values == null || !values.isArray() || values.isEmpty()) {
            ctxt.reportInputMismatch(TransparentEcmqvPublicKey.class, "TransparentEcmqvPublicKey 'value' must be a non-empty array");
            return null;
        }
        ...
        if (!transparentEcmqvPublicKey.isSupported()) {
            throw new NoSuchElementException(String.format("TransparentEcmqvPublicKey is not supported for KMIP spec %s", spec));
        }
