...
public class ObjectTypeJsonDeserializer extends KmipDataTypeJsonDeserializer<ObjectType> {
    ...
    public ObjectType deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (node == null) {
            ctxt.reportInputMismatch(ObjectType.class, String.format("JSON node cannot be null for ObjectType deserialization"));
            return null;
        }
        ...
        try {
            ...
            if (tag == null) {
                ctxt.reportInputMismatch(ObjectType.class, String.format("Invalid KMIP tag for ObjectType"));
                return null;
            }
        } catch (Exception e) {
        ...
        if (!node.isObject() || tag.getValue().getValue() != kmipTag.getValue().getValue()) {
            ctxt.reportInputMismatch(ObjectType.class,
                    String.format("Expected object with %s tag for ObjectType, got tag: %s", kmipTag.getValue().getValue(), tag.getValue().getValue()));
            return null;
        }
        ...
        if (typeNode == null
            ...
            ctxt.reportInputMismatch(ObjectType.class, String.format("Missing or non-text 'type' field for ObjectType"));
            return null;
        }
