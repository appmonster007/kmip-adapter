...
public class KeyMaterialByteStringJsonDeserializer extends KmipDataTypeJsonDeserializer<KeyMaterialByteString> {
    ...
    public KeyMaterialByteString deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (node == null) {
            ctxt.reportInputMismatch(KeyMaterialByteString.class, String.format("JSON node cannot be null for KeyMaterial.ByteString deserialization"));
            return null;
        }
        ...
        try {
            ...
            if (tag == null) {
                ctxt.reportInputMismatch(KeyMaterialByteString.class, String.format("Invalid KMIP tag for KeyMaterial.ByteString"));
                return null;
            }
        } catch (Exception e) {
            ctxt.reportInputMismatch(KeyMaterialByteString.class, String.format("Failed to parse KMIP tag for KeyMaterial.ByteString: %s", e.getMessage()));
            return null;
        }
        ...
        if (typeNode == null
            ...
            ctxt.reportInputMismatch(KeyMaterialByteString.class, String.format("Missing or non-text 'type' field for KeyMaterial.ByteString"));
            return null;
        }
        ...
        if (valueNode == null || !valueNode.isTextual()) {
            ctxt.reportInputMismatch(KeyMaterialByteString.class, "KeyMaterial.ByteString 'value' must be a non-empty array");
            return null;
        }
        ...
        if (!keyMaterialByteString.isSupported()) {
            ctxt.reportInputMismatch(KeyMaterialByteString.class, "KeyMaterial.ByteString not supported for spec " + spec);
            return null;
        }
