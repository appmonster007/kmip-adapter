...
public class EndpointRoleJsonDeserializer extends KmipDataTypeJsonDeserializer<EndpointRole> {
    ...
    public EndpointRole deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (node == null) {
            ctxt.reportInputMismatch(EndpointRole.class, String.format("JSON node cannot be null for EndpointRole deserialization"));
            return null;
        }
        ...
        try {
            ...
            if (tag == null) {
                ctxt.reportInputMismatch(EndpointRole.class, String.format("Invalid KMIP tag for EndpointRole"));
                return null;
            }
        } catch (Exception e) {
        ...
        if (!node.isObject() || tag.getValue().getValue() != kmipTag.getValue().getValue()) {
            ctxt.reportInputMismatch(EndpointRole.class,
                    String.format("Expected object with %s tag for EndpointRole, got tag: %s", kmipTag.getValue().getValue(), tag.getValue().getValue()));
            return null;
        }
        ...
        if (typeNode == null
            ...
            ctxt.reportInputMismatch(EndpointRole.class, String.format("Missing or non-text 'type' field for EndpointRole"));
            return null;
        }
