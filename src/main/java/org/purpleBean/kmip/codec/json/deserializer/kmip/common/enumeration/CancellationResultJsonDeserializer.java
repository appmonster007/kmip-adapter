...
public class CancellationResultJsonDeserializer extends KmipDataTypeJsonDeserializer<CancellationResult> {
    ...
    public CancellationResult deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (node == null) {
            ctxt.reportInputMismatch(CancellationResult.class, String.format("JSON node cannot be null for CancellationResult deserialization"));
            return null;
        }
        ...
        try {
            ...
            if (tag == null) {
                ctxt.reportInputMismatch(CancellationResult.class, String.format("Invalid KMIP tag for CancellationResult"));
                return null;
            }
        } catch (Exception e) {
        ...
        if (!node.isObject() || tag.getValue().getValue() != kmipTag.getValue().getValue()) {
            ctxt.reportInputMismatch(CancellationResult.class,
                    String.format("Expected object with %s tag for CancellationResult, got tag: %s", kmipTag.getValue().getValue(), tag.getValue().getValue()));
            return null;
        }
        ...
        if (typeNode == null
            ...
            ctxt.reportInputMismatch(CancellationResult.class, String.format("Missing or non-text 'type' field for CancellationResult"));
            return null;
        }
