...
public class RevocationMessageTtlvDeserializer extends KmipDataTypeTtlvDeserializer<RevocationMessage> {
    ...
    @Override
    public RevocationMessage deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!revocationMessage.isSupported()) {
            throw new NoSuchElementException(String.format("RevocationMessage not supported for spec %s", spec));
        }
