...
public class MediaIdentifierTtlvDeserializer extends KmipDataTypeTtlvDeserializer<MediaIdentifier> {
    ...
    @Override
    public MediaIdentifier deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!mediaIdentifier.isSupported()) {
            throw new NoSuchElementException(String.format("MediaIdentifier not supported for spec %s", spec));
        }
