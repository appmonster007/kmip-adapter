...
public class BatchCountTtlvDeserializer extends KmipDataTypeTtlvDeserializer<BatchCount> {
    private final KmipTag kmipTag = BatchCount.kmipTag;
    private final EncodingType encodingType = BatchCount.encodingType;

    @Override
    public BatchCount deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!batchCount.isSupported()) {
            throw new NoSuchElementException(String.format("BatchCount not supported for spec %s", spec));
        }
        return batchCount;
    }
}