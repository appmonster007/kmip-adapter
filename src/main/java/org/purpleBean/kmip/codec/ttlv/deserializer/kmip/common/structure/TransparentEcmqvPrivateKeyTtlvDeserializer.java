...
public class TransparentEcmqvPrivateKeyTtlvDeserializer extends KmipDataTypeTtlvDeserializer<TransparentEcmqvPrivateKey> {
    ...
    @Override
    public TransparentEcmqvPrivateKey deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!transparentEcmqvPrivateKey.isSupported()) {
            throw new NoSuchElementException(String.format("%s is not supported for KMIP spec %s", transparentEcmqvPrivateKey.getClass().getSimpleName(), spec));
        }
        return transparentEcmqvPrivateKey;
    }

    private void setValue(TransparentEcmqvPrivateKey.TransparentEcmqvPrivateKeyBuilder builder, KmipTag.Value nodeTag, TtlvObject ttlvObject, TtlvMapper mapper) throws IOException {
        switch (nodeTag) {
