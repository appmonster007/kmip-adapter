...
public class UsageLimitsTotalTtlvDeserializer extends KmipDataTypeTtlvDeserializer<UsageLimitsTotal> {
    ...
    @Override
    public UsageLimitsTotal deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!usageLimitsTotal.isSupported()) {
            throw new NoSuchElementException(String.format("UsageLimitsTotal not supported for spec %s", spec));
        }
