...
public class NetworkIdentifierTtlvDeserializer extends KmipDataTypeTtlvDeserializer<NetworkIdentifier> {
    ...
    @Override
    public NetworkIdentifier deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!networkIdentifier.isSupported()) {
            throw new NoSuchElementException(String.format("NetworkIdentifier not supported for spec %s", spec));
        }
