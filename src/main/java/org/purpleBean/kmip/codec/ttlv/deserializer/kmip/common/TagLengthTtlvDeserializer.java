...
public class TagLengthTtlvDeserializer extends KmipDataTypeTtlvDeserializer<TagLength> {
    private final KmipTag kmipTag = TagLength.kmipTag;
    private final EncodingType encodingType = TagLength.encodingType;

    @Override
    public TagLength deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!tagLength.isSupported()) {
            throw new NoSuchElementException(String.format("TagLength not supported for spec %s", spec));
        }
        return tagLength;
    }
}