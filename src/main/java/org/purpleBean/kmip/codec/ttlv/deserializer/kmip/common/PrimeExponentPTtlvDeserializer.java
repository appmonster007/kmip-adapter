...
public class PrimeExponentPTtlvDeserializer extends KmipDataTypeTtlvDeserializer<PrimeExponentP> {
    private final KmipTag kmipTag = PrimeExponentP.kmipTag;
    private final EncodingType encodingType = PrimeExponentP.encodingType;

    @Override
    public PrimeExponentP deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!primeExponentP.isSupported()) {
            throw new NoSuchElementException(String.format("PrimeExponentP not supported for spec %s", spec));
        }
