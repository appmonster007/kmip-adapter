...
public class ResponseBatchItemTtlvDeserializer extends AbstractKmipDataTypeTtlvDeserializer<ResponseBatchItem, ResponseBatchItem.ResponseBatchItemBuilder> {
    ...
    @Override
    protected void setValue(ResponseBatchItem.ResponseBatchItemBuilder builder, byte[] tag, byte type, ByteBuffer p, TtlvMapper mapper) throws IOException {
        ...
        switch (nodeTag) {
            ...
            case KmipTag.Standard.RESPONSE_PAYLOAD -> builder.responsePayloadStructure(mapper.readValue(p, ResponsePayloadStructure.class));
            case KmipTag.Standard.MESSAGE_EXTENSION -> builder.messageExtension(mapper.readValue(p, MessageExtension.class));
