...
public class MachineIdentifierTtlvDeserializer extends KmipDataTypeTtlvDeserializer<MachineIdentifier> {
    ...
    @Override
    public MachineIdentifier deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!machineIdentifier.isSupported()) {
            throw new NoSuchElementException(String.format("MachineIdentifier not supported for spec %s", spec));
        }
