...
public class InitialCounterValueTtlvDeserializer extends KmipDataTypeTtlvDeserializer<InitialCounterValue> {
    ...
    @Override
    public InitialCounterValue deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!initialCounterValue.isSupported()) {
            throw new NoSuchElementException(String.format("InitialCounterValue not supported for spec %s", spec));
        }
