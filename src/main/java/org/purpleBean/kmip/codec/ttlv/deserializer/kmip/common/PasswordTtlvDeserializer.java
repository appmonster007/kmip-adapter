...
public class PasswordTtlvDeserializer extends KmipDataTypeTtlvDeserializer<Password> {
    private final KmipTag kmipTag = Password.kmipTag;
    private final EncodingType encodingType = Password.encodingType;

    @Override
    public Password deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!password.isSupported()) {
            throw new NoSuchElementException(String.format("Password not supported for spec %s", spec));
        }
        return password;
    }
}