...
public class MaximumItemsTtlvDeserializer extends KmipDataTypeTtlvDeserializer<MaximumItems> {
    private final KmipTag kmipTag = MaximumItems.kmipTag;
    private final EncodingType encodingType = MaximumItems.encodingType;

    @Override
    public MaximumItems deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!maximumItems.isSupported()) {
            throw new NoSuchElementException(String.format("MaximumItems not supported for spec %s", spec));
        }
        return maximumItems;
    }
}