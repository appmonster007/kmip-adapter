...
public final class TtlvObject {
  ...
  private static byte[] serializeMultiple(TtlvObject... ttlvObjects) {
    ...
    int totalLength = Stream
        .of(ttlvObjects)
        .peek(Objects::requireNonNull)
        .mapToInt(o -> calculateTotalLength(o.value.length))
        .sum();
  ...
  public static TtlvObject fromBuffer(ByteBuffer buffer) {
    if (buffer.remaining() < TtlvConstants.HEADER_SIZE) {
      throw new IllegalArgumentException(
          String.format("Insufficient data for TTLV header. Required: %d, available: %d",
    ...
    if (valueLength < 0) {
      throw new IllegalArgumentException(
          String.format(TtlvConstants.ERROR_INVALID_VALUE_LENGTH, valueLength));
    }
    if (buffer.remaining() < valueLength) {
      throw new IllegalArgumentException(
          String.format(TtlvConstants.ERROR_INSUFFICIENT_VALUE_DATA, valueLength,
              buffer.remaining()));
    }
  ...
  private static void writeHeader(ByteBuffer buffer, TtlvObject obj) {
    ...
    if (tag.length != TtlvConstants.TAG_SIZE) {
      throw new IllegalArgumentException(
          String.format("Tag must be %d bytes, got %d", TtlvConstants.TAG_SIZE, tag.length));
    }
    buffer.put(tag);
    buffer.put(obj.type);
  ...
  private static void skipPadding(ByteBuffer buffer, int valueLength) {
    ...
    if (paddingToSkip > 0) {
      if (buffer.remaining() < paddingToSkip) {
        throw new IllegalArgumentException(
            String.format(TtlvConstants.ERROR_INSUFFICIENT_PADDING_DATA, paddingToSkip,
                buffer.remaining()));
      }
  ...
  public List<TtlvObject> getNestedValue() {
    if (!isStructure()) {
      throw new IllegalStateException(
          "This object does not contain nested values. Use getPrimitiveValue().");
    }
  ...
  private String getStructuredByteStringAtLevel(int level) {
    ...
    sb
        .append(indent)
        .append(hexFormat.formatHex(tag))
        .append(" ")
        .append(hexFormat.toHexDigits(type))
        .append(" ")
        .append(hexFormat.toHexDigits(length));
    ...
    if (!hasEmptyValue()) {
      if (type == EncodingType.STRUCTURE.getTypeValue()) {
        ...
        sb
            .append("\n")
            .append(indent)
            .append(hexFormat.formatHex(value));
      }
    }
    return sb.toString();
  }
