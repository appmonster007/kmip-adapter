...
public class SplitKeyThresholdTtlvDeserializer extends KmipDataTypeTtlvDeserializer<SplitKeyThreshold> {
    ...
    @Override
    public SplitKeyThreshold deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!splitKeyThreshold.isSupported()) {
            throw new NoSuchElementException(String.format("SplitKeyThreshold not supported for spec %s", spec));
        }
