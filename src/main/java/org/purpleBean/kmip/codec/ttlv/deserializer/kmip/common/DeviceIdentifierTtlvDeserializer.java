...
public class DeviceIdentifierTtlvDeserializer extends KmipDataTypeTtlvDeserializer<DeviceIdentifier> {
    ...
    @Override
    public DeviceIdentifier deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!deviceIdentifier.isSupported()) {
            throw new NoSuchElementException(String.format("DeviceIdentifier not supported for spec %s", spec));
        }
