...
public class FixedFieldLengthTtlvDeserializer extends KmipDataTypeTtlvDeserializer<FixedFieldLength> {
    ...
    @Override
    public FixedFieldLength deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!fixedFieldLength.isSupported()) {
            throw new NoSuchElementException(String.format("FixedFieldLength not supported for spec %s", spec));
        }
