...
public class ApplicationNamespaceTtlvDeserializer extends KmipDataTypeTtlvDeserializer<ApplicationNamespace> {
    ...
    @Override
    public ApplicationNamespace deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!applicationNamespace.isSupported()) {
            throw new NoSuchElementException(String.format("ApplicationNamespace not supported for spec %s", spec));
        }
