...
public class ExtensionNameTtlvDeserializer extends KmipDataTypeTtlvDeserializer<ExtensionName> {
    private final KmipTag kmipTag = ExtensionName.kmipTag;
    private final EncodingType encodingType = ExtensionName.encodingType;

    @Override
    public ExtensionName deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!extensionName.isSupported()) {
            throw new NoSuchElementException(String.format("ExtensionName not supported for spec %s", spec));
        }
