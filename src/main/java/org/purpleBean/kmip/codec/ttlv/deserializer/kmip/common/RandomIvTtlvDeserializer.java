...
public class RandomIvTtlvDeserializer extends KmipDataTypeTtlvDeserializer<RandomIv> {
    private final KmipTag kmipTag = RandomIv.kmipTag;
    private final EncodingType encodingType = RandomIv.encodingType;

    @Override
    public RandomIv deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!randomIv.isSupported()) {
            throw new NoSuchElementException(String.format("RandomIv not supported for spec %s", spec));
        }
        return randomIv;
    }
}