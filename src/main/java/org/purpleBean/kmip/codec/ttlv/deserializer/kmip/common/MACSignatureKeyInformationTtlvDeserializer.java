...
public class MACSignatureKeyInformationTtlvDeserializer extends KmipDataTypeTtlvDeserializer<MACSignatureKeyInformation> {
    ...
    @Override
    public MACSignatureKeyInformation deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!mACSignatureKeyInformation.isSupported()) {
            throw new NoSuchElementException(String.format("MACSignatureKeyInformation not supported for spec %s", spec));
        }
