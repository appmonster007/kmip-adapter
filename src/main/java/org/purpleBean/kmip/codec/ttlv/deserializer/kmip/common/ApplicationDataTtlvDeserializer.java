...
public class ApplicationDataTtlvDeserializer extends KmipDataTypeTtlvDeserializer<ApplicationData> {
    ...
    @Override
    public ApplicationData deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!applicationData.isSupported()) {
            throw new NoSuchElementException(String.format("ApplicationData not supported for spec %s", spec));
        }
