...
public class InvocationFieldLengthTtlvDeserializer extends KmipDataTypeTtlvDeserializer<InvocationFieldLength> {
    ...
    @Override
    public InvocationFieldLength deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!invocationFieldLength.isSupported()) {
            throw new NoSuchElementException(String.format("InvocationFieldLength not supported for spec %s", spec));
        }
