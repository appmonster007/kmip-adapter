...
public class DeviceSerialNumberTtlvDeserializer extends KmipDataTypeTtlvDeserializer<DeviceSerialNumber> {
    ...
    @Override
    public DeviceSerialNumber deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!deviceSerialNumber.isSupported()) {
            throw new NoSuchElementException(String.format("DeviceSerialNumber not supported for spec %s", spec));
        }
