...
public class PublicExponentTtlvDeserializer extends KmipDataTypeTtlvDeserializer<PublicExponent> {
    private final KmipTag kmipTag = PublicExponent.kmipTag;
    private final EncodingType encodingType = PublicExponent.encodingType;

    @Override
    public PublicExponent deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!publicExponent.isSupported()) {
            throw new NoSuchElementException(String.format("PublicExponent not supported for spec %s", spec));
        }
