...
public class LinkedObjectIdentifierTtlvDeserializer extends KmipDataTypeTtlvDeserializer<LinkedObjectIdentifier> {
    ...
    @Override
    public LinkedObjectIdentifier deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!linkedObjectIdentifier.isSupported()) {
            throw new NoSuchElementException(String.format("LinkedObjectIdentifier not supported for spec %s", spec));
        }
