...
public class TransparentEcdsaPublicKeyTtlvDeserializer extends KmipDataTypeTtlvDeserializer<TransparentEcdsaPublicKey> {
    ...
    @Override
    public TransparentEcdsaPublicKey deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!transparentEcdsaPublicKey.isSupported()) {
            throw new NoSuchElementException(String.format("%s is not supported for KMIP spec %s", transparentEcdsaPublicKey.getClass().getSimpleName(), spec));
        }
        return transparentEcdsaPublicKey;
    }

    private void setValue(TransparentEcdsaPublicKey.TransparentEcdsaPublicKeyBuilder builder, KmipTag.Value nodeTag, TtlvObject ttlvObject, TtlvMapper mapper) throws IOException {
        switch (nodeTag) {
