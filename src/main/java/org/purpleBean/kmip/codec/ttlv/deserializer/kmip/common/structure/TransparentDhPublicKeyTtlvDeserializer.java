...
public class TransparentDhPublicKeyTtlvDeserializer extends KmipDataTypeTtlvDeserializer<TransparentDhPublicKey> {
    ...
    @Override
    public TransparentDhPublicKey deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!transparentDhPublicKey.isSupported()) {
            throw new NoSuchElementException(String.format("%s is not supported for KMIP spec %s", transparentDhPublicKey.getClass().getSimpleName(), spec));
        }
        return transparentDhPublicKey;
    }

    private void setValue(TransparentDhPublicKey.TransparentDhPublicKeyBuilder builder, KmipTag.Value nodeTag, TtlvObject ttlvObject, TtlvMapper mapper) throws IOException {
        switch (nodeTag) {
