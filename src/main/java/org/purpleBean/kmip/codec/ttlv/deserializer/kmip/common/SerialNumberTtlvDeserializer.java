...
public class SerialNumberTtlvDeserializer extends KmipDataTypeTtlvDeserializer<SerialNumber> {
    private final KmipTag kmipTag = SerialNumber.kmipTag;
    private final EncodingType encodingType = SerialNumber.encodingType;

    @Override
    public SerialNumber deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!serialNumber.isSupported()) {
            throw new NoSuchElementException(String.format("SerialNumber not supported for spec %s", spec));
        }
        return serialNumber;
    }
}