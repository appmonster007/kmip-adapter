...
public class ExtensionTypeTtlvDeserializer extends KmipDataTypeTtlvDeserializer<ExtensionType> {
    private final KmipTag kmipTag = ExtensionType.kmipTag;
    private final EncodingType encodingType = ExtensionType.encodingType;

    @Override
    public ExtensionType deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!extensionType.isSupported()) {
            throw new NoSuchElementException(String.format("ExtensionType not supported for spec %s", spec));
        }
