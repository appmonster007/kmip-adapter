...
public class BatchOrderOptionTtlvDeserializer extends KmipDataTypeTtlvDeserializer<BatchOrderOption> {
    ...
    @Override
    public BatchOrderOption deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!batchOrderOption.isSupported()) {
            throw new NoSuchElementException(String.format("BatchOrderOption not supported for spec %s", spec));
        }
