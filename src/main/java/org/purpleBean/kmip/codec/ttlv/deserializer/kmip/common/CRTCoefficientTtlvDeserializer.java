...
public class CRTCoefficientTtlvDeserializer extends KmipDataTypeTtlvDeserializer<CRTCoefficient> {
    private final KmipTag kmipTag = CRTCoefficient.kmipTag;
    private final EncodingType encodingType = CRTCoefficient.encodingType;

    @Override
    public CRTCoefficient deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!cRTCoefficient.isSupported()) {
            throw new NoSuchElementException(String.format("CRTCoefficient not supported for spec %s", spec));
        }
