...
public class DataLengthTtlvDeserializer extends KmipDataTypeTtlvDeserializer<DataLength> {
    private final KmipTag kmipTag = DataLength.kmipTag;
    private final EncodingType encodingType = DataLength.encodingType;

    @Override
    public DataLength deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!dataLength.isSupported()) {
            throw new NoSuchElementException(String.format("DataLength not supported for spec %s", spec));
        }
        return dataLength;
    }
}
