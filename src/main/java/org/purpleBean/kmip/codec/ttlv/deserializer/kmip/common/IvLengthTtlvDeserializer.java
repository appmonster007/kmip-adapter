...
public class IvLengthTtlvDeserializer extends KmipDataTypeTtlvDeserializer<IvLength> {
    private final KmipTag kmipTag = IvLength.kmipTag;
    private final EncodingType encodingType = IvLength.encodingType;

    @Override
    public IvLength deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!ivLength.isSupported()) {
            throw new NoSuchElementException(String.format("IvLength not supported for spec %s", spec));
        }
        return ivLength;
    }
}