...
public class IssuerDistinguishedNameTtlvDeserializer extends KmipDataTypeTtlvDeserializer<IssuerDistinguishedName> {
    ...
    @Override
    public IssuerDistinguishedName deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!issuerDistinguishedName.isSupported()) {
            throw new NoSuchElementException(String.format("IssuerDistinguishedName not supported for spec %s", spec));
        }
