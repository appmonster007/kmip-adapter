...
public class QlengthTtlvDeserializer extends KmipDataTypeTtlvDeserializer<Qlength> {
    private final KmipTag kmipTag = Qlength.kmipTag;
    private final EncodingType encodingType = Qlength.encodingType;

    @Override
    public Qlength deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!qlength.isSupported()) {
            throw new NoSuchElementException(String.format("Qlength not supported for spec %s", spec));
        }
        return qlength;
    }
}