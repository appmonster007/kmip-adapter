...
public class FreshTtlvDeserializer extends KmipDataTypeTtlvDeserializer<Fresh> {
    private final KmipTag kmipTag = Fresh.kmipTag;
    private final EncodingType encodingType = Fresh.encodingType;

    @Override
    public Fresh deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!fresh.isSupported()) {
            throw new NoSuchElementException(String.format("Fresh not supported for spec %s", spec));
        }
        return fresh;
    }
}