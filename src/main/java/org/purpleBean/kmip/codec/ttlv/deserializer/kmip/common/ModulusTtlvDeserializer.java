...
public class ModulusTtlvDeserializer extends KmipDataTypeTtlvDeserializer<Modulus> {
    private final KmipTag kmipTag = Modulus.kmipTag;
    private final EncodingType encodingType = Modulus.encodingType;

    @Override
    public Modulus deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!modulus.isSupported()) {
            throw new NoSuchElementException(String.format("Modulus not supported for spec %s", spec));
        }
        return modulus;
    }
}