...
public class MaximumResponseSizeTtlvDeserializer extends KmipDataTypeTtlvDeserializer<MaximumResponseSize> {
    ...
    @Override
    public MaximumResponseSize deserialize(ByteBuffer ttlvBuffer, TtlvMapper mapper) throws IOException {
        ...
        if (!maximumResponseSize.isSupported()) {
            throw new NoSuchElementException(String.format("MaximumResponseSize not supported for spec %s", spec));
        }
