...
public class LinkXmlSerializer extends KmipDataTypeXmlSerializer<Link> {
    ...
    public void serialize(Link link, JsonGenerator gen, SerializerProvider serializers) throws IOException {
        ...
        if (!link.isSupported()) {
            throw new UnsupportedEncodingException(String.format("%s not supported for KMIP spec %s", link.getClass().getSimpleName(), spec));
        }
        ...
        String elementName = link.getKmipTag().getDescription();
        xmlGen.setNextName(QName.valueOf(elementName));
        xmlGen.writeStartObject(link);

        // Serialize all fields
        List<KmipDataType> values = link.getValues();
