package org.purpleBean.kmip.codec.xml.deserializer.model.v2_1.structure.request;
...
public class RequestHeaderXmlDeserializer extends AbstractKmipDataTypeXmlDeserializer<RequestHeader, RequestHeader.RequestHeaderBuilder> {
    ...
    @Override
    protected void setValue(RequestHeader.RequestHeaderBuilder builder, String tag, String type, JsonParser p, DeserializationContext ctxt) throws IOException {
        KmipTag.Value nodeTag = KmipTag.fromName(tag);
        switch (nodeTag) {
            case KmipTag.Standard.PROTOCOL_VERSION -> builder.protocolVersion(ctxt.readValue(p, ProtocolVersion.class));
            case KmipTag.Standard.MAXIMUM_RESPONSE_SIZE -> builder.maximumResponseSize(ctxt.readValue(p, MaximumResponseSize.class));
            case KmipTag.Standard.CLIENT_CORRELATION_VALUE -> builder.clientCorrelationValue(ctxt.readValue(p, ClientCorrelationValue.class));
            ...
            case KmipTag.Standard.ASYNCHRONOUS_INDICATOR -> builder.asynchronousIndicator(ctxt.readValue(p, AsynchronousIndicator.class));
            case KmipTag.Standard.ATTESTATION_CAPABLE_INDICATOR -> builder.attestationCapableIndicator(ctxt.readValue(p, AttestationCapableIndicator.class));
            ...
            case KmipTag.Standard.BATCH_ERROR_CONTINUATION_OPTION -> builder.batchErrorContinuationOption(ctxt.readValue(p, BatchErrorContinuationOption.class));
            case KmipTag.Standard.BATCH_ORDER_OPTION -> builder.batchOrderOption(ctxt.readValue(p, BatchOrderOption.class));
