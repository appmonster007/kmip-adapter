...
/**
 * XML deserializer for {@link BigInteger}.
 */
public class BigIntegerXmlDeserializer extends JsonDeserializer<BigInteger> {
  ...
  public BigInteger deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
    ...
    try {
      ...
      throw new IllegalArgumentException("Invalid hex string for BigInteger: '" + hexString + "'",
          e);
    }

    if (!TtlvConstants.isProperlyPadded(data.length)) {
      throw new IllegalArgumentException(
          "Expected 8n bytes to get value, got " + data.length + " bytes");
    }
