 ...
 * <p><b>Standards:</b></p>
 * <ul>
 *   <li>{@code SUBMITTED}: The operation has been submitted but has not yet started processing
 *   .</li>
 *   <li>{@code IN_PROCESS}: The operation is currently being processed.</li>
 *   <li>{@code COMPLETED}: The operation has completed.</li>
 * </ul>
 *
 * @see KmipEnumeration
 * @see AsynchronousIndicator
 */
@Data
@Builder(toBuilder = true)
public class ProcessingStage implements KmipEnumeration {
  public static final KmipTag kmipTag = KmipTag.Standard.PROCESSING_STAGE.inst();
  ...
  static {
    ...
    for (KmipSpec spec : supportedVersions) {
      ...
      KmipDataType.register(spec, kmipTag.getValue(), encodingType, ProcessingStage.class);
      KmipEnumeration.register(spec, kmipTag.getValue(), ProcessingStage::fromName,
          ProcessingStage::fromValue);
    }
  }
  ...
  public static Value fromName(String name) {
    ...
    return Optional
        ...
        .orElseThrow(() -> new NoSuchElementException(
            String.format("No ProcessingStage value found for '%s' in KMIP spec %s", name, spec)
        ));
  }
  ...
  public static Value fromValue(int value) {
    ...
    return Optional
        ...
        .filter(Value::isSupported)
        .orElseThrow(() -> new NoSuchElementException(
            String.format("No ProcessingStage value found for %d in KMIP spec %s", value, spec)
        ));
  }
  ...
  private void validate() {
    ...
    if (!value.isSupported()) {
      throw new IllegalArgumentException(
          String.format("Value '%s' for ProcessingStage is not supported for KMIP spec %s",
  ...
  /**
   * The standard enumeration of Processing Stages.
   */
  @Getter
  @AllArgsConstructor
  @ToString
  public enum Standard implements Value {
    ...
    @Override
    public ProcessingStage inst() {
      return ProcessingStage.of(this);
    }
  }

  /**
   * An interface representing a Processing Stage value, which can be either a standard
   * value or a custom extension.
   */
  public interface Value extends KmipEnumeration.Value<ProcessingStage> {
  }

  /**
   * Represents a custom, vendor-specific Processing Stage.
   */
  @Getter
  @AllArgsConstructor
  @ToString
