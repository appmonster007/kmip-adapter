...
public class RegisterOpRequestPayload implements RequestPayloadStructure {
  ...
  static {
    for (KmipSpec spec : supportedVersions) {
      ...
      RequestPayloadStructure.register(spec, operation, RegisterOpRequestPayload.class,
          RegisterOpRequestPayload::of);
    }
  }
  ...
  public static RegisterOpRequestPayload of(List<KmipDataType> values) {
    ...
    // The object itself doesn't have a fixed tag, so we need to find the one that is not
    // ObjectType or TemplateAttribute
    values
        .stream()
        .filter(v -> !v
            .getKmipTag()
