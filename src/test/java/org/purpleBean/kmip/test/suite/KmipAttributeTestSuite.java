...
public interface KmipAttributeTestSuite<T extends KmipAttribute> {
    ...
    /**
     * @return A {@link State} for which {@link KmipAttribute#isServerModifiable(State)} should return {@code true}.
     */
    State stateForServerModifiableTrue();

    /**
     * @return A {@link State} for which {@link KmipAttribute#isServerModifiable(State)} should return {@code false}.
     */
    State stateForServerModifiableFalse();

    /**
     * @return A {@link State} for which {@link KmipAttribute#isClientModifiable(State)} should return {@code true}.
     */
    State stateForClientModifiableTrue();

    /**
     * @return A {@link State} for which {@link KmipAttribute#isClientModifiable(State)} should return {@code false}.
     */
    ...
    default String expectedAttributeName() {
        return StringUtils.convertPascalToTitleCase(createDefault().getKmipTag().getDescription());
    }
    ...
    default void attribute_capabilityFlags_matchExpectations() {
        ...
        assertThat(attr.isAlwaysPresent()).as("isAlwaysPresent").isEqualTo(expectAlwaysPresent());
        assertThat(attr.isServerInitializable()).as("isServerInitializable").isEqualTo(expectServerInitializable());
        assertThat(attr.isClientInitializable()).as("isClientInitializable").isEqualTo(expectClientInitializable());
        assertThat(attr.isClientDeletable()).as("isClientDeletable").isEqualTo(expectClientDeletable());
        assertThat(attr.isMultiInstanceAllowed()).as("isMultiInstanceAllowed").isEqualTo(expectMultiInstanceAllowed());
    }
    ...
    default void attribute_serverModifiable_respectsState() {
        T attr = createDefault();
        assertThat(attr.isServerModifiable(stateForServerModifiableTrue())).as("isServerModifiable(true state)").isTrue();
        assertThat(attr.isServerModifiable(stateForServerModifiableFalse())).as("isServerModifiable(false state)").isFalse();
    }
    ...
    default void attribute_clientModifiable_respectsState() {
        T attr = createDefault();
        assertThat(attr.isClientModifiable(stateForClientModifiableTrue())).as("isClientModifiable(true state)").isTrue();
        assertThat(attr.isClientModifiable(stateForClientModifiableFalse())).as("isClientModifiable(false state)").isFalse();
    }
    ...
    default void attribute_roundTrip() {
        ...
        assertThat(reconstructed).as("Reconstructed attribute").isNotNull();
        assertThat(reconstructed.getAttributeValue()).as("AttributeValue equality").isEqualTo(obj.getAttributeValue());
        assertThat(reconstructed.getAttributeName()).as("AttributeName equality").isEqualTo(obj.getAttributeName());
    }
    ...
    default void attribute_getAttributeValue_returnsExpectedValue() {
        T attr = createDefault();
        assertThat(attr.getAttributeValue()).as("AttributeValue").isEqualTo(expectedAttributeValue());
    }
    ...
    default void attribute_getAttributeName_returnsExpectedValue() {
        T attr = createDefault();
        assertThat(attr.getAttributeName()).as("AttributeName").isNotNull();
        assertThat(attr.getAttributeName().getValue()).as("AttributeName value").isEqualTo(expectedAttributeName());
    }
    ...
    default void attribute_getCanonicalName_returnsExpectedValue() {
        T attr = createDefault();
        assertThat(attr.getCanonicalName()).as("CanonicalName").isEqualTo(attr.getKmipTag().getDescription());
    }
}
