...
class CreateKeyPairOpResponsePayloadTest extends AbstractKmipStructureTestSuite<CreateKeyPairOpResponsePayload> {
    ...
    protected void validateComponents(List<KmipDataType> values) {
        assertThat(values.stream().filter(v -> v instanceof PrivateKeyUniqueIdentifier).count()).isEqualTo(1);
        assertThat(values.stream().filter(v -> v instanceof PublicKeyUniqueIdentifier).count()).isEqualTo(1);
    }
}
